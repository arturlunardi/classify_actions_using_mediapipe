    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        while cap.isOpened():
            ...
            if ret == True:
                ...
                try:
                    # num_coords = len(results.pose_landmarks.landmark) +  len(results.face_landmarks.landmark) + len(results.right_hand_landmarks.landmark) + len(results.left_hand_landmarks.landmark)# num_coords: 33
...
if __name__ == '__main__':
    ...
    dataset_csv_file = os.path.abspath(os.path.join(__file__, utils._root_data_path, utils._coords_dataset_file))
    videos_train_path = os.path.abspath(os.path.join(__file__, utils._root_data_path, utils._video_training_path))

    # for this example we chose to work only with pose_landmarks values because it is easier to detect
    # if you want to add more positions like face, right_hand, left_hand etc you may add it
    # go to the add_records_coordinates and the create_pose_csv functions, compute and add to the row variable
    # the num_cords variable on create_pose_csv function needs to be modified too
    # just a reminder that if a frame can't identify every landmark, it will not be computed

    # checking if the dataset coordinates exist, if not, create it
    if os.path.isfile(dataset_csv_file):
        print (f'{dataset_csv_file}: Exist.')

        # run through every video and compute the coordinates on the csv
        for video in os.listdir(videos_train_path):
            cap = cv2.VideoCapture(os.path.join(videos_train_path, video))
            add_class = video[:-4]
